...
def run():
    ...
    while traci.simulation.getMinExpectedNumber() > 0:
        ...
        if "0" in traci.vehicle.getIDList(): 
            traci.vehicle.setAcceleration("0", 2, 100)
        if "1" in traci.vehicle.getIDList():
            traci.vehicle.setAcceleration("1", 2, 100)
        if "test" in traci.vehicle.getIDList():
            traci.vehicle.setAcceleration("test", 2, 100)
        if "2" in traci.vehicle.getIDList():
            traci.vehicle.setAcceleration("2", 2, 100)
        if "3" in traci.vehicle.getIDList():
            traci.vehicle.setAcceleration("3", 2, 100)     
